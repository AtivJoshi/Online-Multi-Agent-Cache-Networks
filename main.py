...
if __name__ == '__main__':
    ...
    caches_I = list(set(np.argsort(list(dict(graph.degree).values()))).difference(sources)) # list of cache nodes except the sources

    # assign an owner(agent) to each cache node exept the source nodes
    if args.graph_type == 'balanced_tree':
    ...
    traces = [] # Each element is a 2d array of file request. A file request for time slot t contains an array whose ith element denotes the number of times file i was requested in that timeslot.
    for trace_loc in args.traces:
        if trace_loc == '':
            continue
        with open(trace_loc, 'rb') as f:
            traces.append(pickle.load(f))
    query_nodes_trace = dict([(q, traces[owners[q] % len(traces)]) for q in query_nodes]) # assigning traces to each query node
